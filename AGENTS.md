...
La fase actual, autorizada el 22/09/2026, prepara el backend Node/Express para un
piloto posterior en Render Free + Neon Free. Conservar JPG/JPEG/PNG/PDF y la sonda
Workers intacta; no continuar Cloudflare. No crear cuentas, bases, recursos,
secretos reales, despliegues ni facturación. Ver backend/DEPLOYMENT.md y render.yaml.
...
- QuotaStore desacoplado: MemoryQuotaStore para pruebas/mock, SQLiteQuotaStore local y PostgresQuotaStore para Render/Neon. QUOTA_STORE selecciona; ausente conserva memory en mock y sqlite en gemini. DATABASE_URL es secreto solo del backend, nunca Expo, Git ni logs. PostgreSQL falla cerrado sin fallback, usa pg con TLS verificado, pool máximo 2 y migraciones transaccionales registradas. No cargar SQLite ni crear su carpeta en modo postgres. Purga PostgreSQL al iniciar/admitir solicitudes, sin consultas periódicas que impidan suspensión. Contadores diarios y pausa sobreviven reinicios; IP/concurrencia en memoria, no coordinados entre procesos. No para producción; antes de pruebas públicas, sacar la base de datos de carpetas sincronizadas.
...
- CORS con orígenes exactos, sin comodines, móvil sin Origin permitido. trust proxy desactivado por defecto; solo TRUST_PROXY_CIDRS explícitos y verificados, nunca confiar en todos ni adivinar saltos de Render. La IP puede agrupar usuarios detrás del proxy; no persiste. HOST local predeterminado; Render usa 0.0.0.0 y PORT del entorno.
- Conservar el diseño aprobado, estados vacíos, errores, cancelación y accesibilidad.

## Comprobaciones

Para esta fase sin cambios Expo: tipos, ESLint, pruebas focalizadas y suite completa una vez, compilación backend, Expo Doctor y revisión de secretos. No repetir exportaciones. No conectar pruebas a Neon o Gemini; los dobles no demuestran integración remota. Crear commit solo tras comprobar lo necesario.
